...
# make an empty graph
plot(NA, NA, ylim = c(0,1), xlim = c(-.2, .1), yaxt = "n", ylab = NA, xlab = "Difference in mean blood mercury (ug/g)", main = "Mercury in seals that forage \n in northern vs. southern locations")
...
# a claim: we can reject the hypothesis that there is no difference in means (the confidence interval does not cross the 0 line); seals that forage in southern waters have more mercury in their blood than seals that forage in northern waters.
