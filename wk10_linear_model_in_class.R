...
sqr <- read.csv("data/squirrel_data.csv")

# look at the structure of the data
str(sqr)

# assign x and y variables to the predictor and response variables, respectively 
Predictor <- sqr$parent_midpoint
Response <- sqr$offspring_rattleprob
...
# none of causation fulfilled by today 
...
new_predictor <- seq(min_pred, max_pred, length.out = 1000)
...
lines(new_predictor, pred_Y, lwd=2, col = "blue")
...
lines(new_predictor,predicted[ ,2],lty=3, col = "blue")
lines(new_predictor,predicted[ ,3],lty=3, col = "blue")
